return {
  setup = function(ops)
    ops = vim.tbl_deep_extend('force', require 'yoelvp.lspconfig.default', ops)
    ...
    for server, options in pairs(ops.servers) do
      if options.enable == true then
        local ok, definition = pcall(require, 'yoelvp.lspconfig.servers.' .. server)
    ...
    require('yoelvp.lspconfig.null-ls')
    ...
    vim.api.nvim_create_user_command('YoelvpLSP', function()
      vim.ui.select(servers, { prompt = 'LSP Servers' }, function(server)
        ...
        local path = string.format('%s/lua/yoelvp/lspconfig/servers/%s.lua', vim.fn.stdpath 'config', server)
